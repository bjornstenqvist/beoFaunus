...
namespace Faunus
{
    ...
    namespace Move
    {
         ...
         */
        template<class Tspace>
            class TranslateRotateLarge : public Movebase<Tspace>
        {
            private:
                typedef Movebase<Tspace> base;
            protected:
                ...
                Point dir2;         //!< Rotational axis directions (default: x=y=z=1). This will be set by setGroup()
                bool center_rotation;

            public:

                TranslateRotateLarge( Energy::Energybase<Tspace> &, Tspace &, Tmjson & );
                ...
                std::map<string, Point> directions2; //!< Specify special group rotational axis directions (default: x=y=z=1)
        };
         ...
         * :--------- | :-------------------------------------------------------------
         * `dir`      | Move directions (default: "1 1 1" = xyz)
         * `dir2`     | Rotational directions (default: "1 1 1" = xyz)
         ...
         */
        template<class Tspace>
            TranslateRotateLarge<Tspace>::TranslateRotateLarge(
                    Energy::Energybase<Tspace> &e,
                    Tspace &s, Tmjson &j ) : base(e, s)
        {

            base::title = "Group Rotation/Translation Large";
            base::w = 30;
            igroup = nullptr;
        ...
        template<class Tspace>
            void TranslateRotateLarge<Tspace>::setGroup( Group &g )
            {
                ...
                if ( directions.find(g.name) != directions.end())
                    ...
                    dir.x() = dir.y() = dir.z() = 1;
                if ( directions2.find(g.name) != directions2.end())
                    dir2 = directions2[g.name];
                else
                    dir2.x() = dir2.y() = dir2.z() = 1;
            }

        template<class Tspace>
            void TranslateRotateLarge<Tspace>::_trialMove()
            {
                ...
                if ( dp_rot > 1e-6 )
                {
                    ...
                    //Point tmp(0,0,1);
                    p.ranunit(slump,dir2);             // random unit vector
        ...
        template<class Tspace>
            void TranslateRotateLarge<Tspace>::_acceptMove()
            {
        ...
        template<class Tspace>
            void TranslateRotateLarge<Tspace>::_rejectMove()
            {
        ...
        template<class Tspace>
            double TranslateRotateLarge<Tspace>::_energyChange()
            {
        ...
        template<class Tspace>
            string TranslateRotateLarge<Tspace>::_info()
            {
                using namespace textio;
                std::ostringstream o;
                ...
                if ( !directions2.empty())
                {
                    o << indent(SUB) << "Group Rotational directions:" << endl;
                    for ( auto &m : directions2 )
                        o << pad(SUBSUB, w - 2, m.first)
                            << m.second.transpose() << endl;
                }
                if ( cnt > 0 )
        ...
        template<class Tspace>
            Tmjson TranslateRotateLarge<Tspace>::_json()
            {
                using namespace textio;
                Tmjson j;
                j = {
        ...
        template<class Tspace>
            void TranslateRotateLarge<Tspace>::_test( UnitTest &t )
            {
                    ...
                    Energy::Energybase<Tspace> &e, Tspace &s, Tmjson &j ) : base(e, s, j)
            {
                ...
                else
                {
                    for ( auto &i : this->mollist )
                    {
                        ...
                        dir2 << j.value("dir2", string("1 1 1") );  // magic!
...
#pragma omp parallel for reduction (+:du)
            ...
            class ClusterMove : public TranslateRotate<Tspace>
        {
            protected:
                ...
                string _info() override;
                Average<double> avgsize; //!< Average number of ions in cluster
                Average<double> avgsizeA; //!< Average number of ions in cluster
                Average<double> avgbias; //!< Average bias
        ...
        template<class Tspace>
                    ...
                    Energy::Energybase<Tspace> &e, Tspace &s, Tmjson &j ) : base(e, s, j)
            {
                ...
                for ( auto &i : this->mollist )
                {
                    ...
                    if(m[centername]["staticmol"].size()>0) {
                        if(m[centername]["staticmol"].is_array()) {
                            for(auto &name : m[centername]["staticmol"]) {
                    ...
                    temp  << j.value("dir2", string("1 1 1"));
                    dir2.at(int(i.first)) = temp;  // magic!
                }
            }
        ...
        template<class Tspace>
            string ClusterMove<Tspace>::_info()
            {
                ...
                {
                    o << pad(SUB, w, "Average cluster size") << avgsize.avg() << endl;
		    o << pad(SUB, w, "Std. of cluster size") << avgsize.stdev() << endl;
		    o << pad(SUB, w, "AverageA cluster size") << avgsizeA.avg() << endl;
		    o << pad(SUB, w, "Std. of clusterA size") << avgsizeA.stdev() << endl;
		    o << pad(SUB, w, "Number of clusters") << avgsizeA.cnt << endl;
		...
		avgsizeA += cindex.size();
			...
			p.ranunit(slump,dir2.at(int(this->currentMolId)));
                        for ( auto i : cindex )
                ...
                if ( dp_trans.at(int(this->currentMolId)) > 1e-6 ) {
                    Point u;
                    u.ranunit(slump,dir.at(int(this->currentMolId)));
...
#pragma omp parallel for reduction (+:du)
        ...
        template<class Tspace>
            ClusterMove2<Tspace>::ClusterMove2(
                    Energy::Energybase<Tspace> &e, Tspace &s, Tmjson &j ) : base(e, s, j)
            {
                base::title = "Cluster2 " + base::title;
                base::cite = "doi:10/cj9gnn";
                gstatic.resize(spc->molecule.size());
                threshold.resize(spc->molecule.size());
                dp_rot.resize(spc->molecule.size());
        ...
        template<class Tspace>
            void ClusterMove2<Tspace>::getClusterAroundMolecule(Group *g)
            {
                int cntI = 0;
                for ( auto &i : spc->molList() ) { 	// For every type of molecule ...
                    bool is_static = false; 	// Start to assume the molecule is not prohibited from being in cluster around molecule 'g'
...
#pragma omp parallel for reduction (+:du)
        ...
        template<class Tspace>
            class ClusterMove3 : public TranslateRotate<Tspace>
        {
            protected:
                typedef TranslateRotate<Tspace> base;
                typedef typename Tspace::ParticleVector Tpvec;
            ...
            public:
                using base::spc;
                ClusterMove3( Energy::Energybase<Tspace> &, Tspace &, Tmjson &j );
                virtual ~ClusterMove3();
                vector<double> threshold;        //!< Distance between particles to define a cluster
        };

        template<class Tspace>
            ClusterMove3<Tspace>::ClusterMove3(
                    Energy::Energybase<Tspace> &e, Tspace &s, Tmjson &j ) : base(e, s, j)
            {
                base::title = "Cluster3 " + base::title;
                base::cite = "doi:10/cj9gnn";
                gstatic.resize(spc->molecule.size());
                threshold.resize(spc->molecule.size());
                dp_rot.resize(spc->molecule.size());
        ...
        template<class Tspace>
            string ClusterMove3<Tspace>::_info()
            {
                using namespace textio;
                std::ostringstream o;
                o << base::_info() << endl;
        ...
        template<class Tspace>
            void ClusterMove3<Tspace>::getClusterAroundMolecule(Group *g)
            {
                int cntI = 0;
                for ( auto &i : spc->molList() ) { 	// For every type of molecule ...
                    bool is_static = false; 	// Start to assume the molecule is not prohibited from being in cluster around molecule 'g'
        ...
        template<class Tspace>
            void ClusterMove3<Tspace>::_trialMove()
            {
                // if `mollist` has data, favor this over `setGroup()`
                // Note that `currentMolId` is set by Movebase::move()
                if ( !this->mollist.empty())
        ...
        template<class Tspace>
            void ClusterMove3<Tspace>::_acceptMove()
            {
                base::_acceptMove();
                for ( auto k : cindex ) {
                    k->accept(*spc);
        ...
        template<class Tspace>
            void ClusterMove3<Tspace>::_rejectMove()
            {
                base::_rejectMove();
                for ( auto k : cindex ) {
                    k->undo(*spc);
        ...
        template<class Tspace>
            double ClusterMove3<Tspace>::_energyChange()
            {
                double bias = 1;             // cluster bias -- see Frenkel 2nd ed, p.405
...
#pragma omp parallel for reduction (+:du)
         ...
         */
        template<class Tspace>
            double ClusterMove3<Tspace>::ClusterProbability(Group &centergroup, Tpvec &p, int i )
            {
                for ( auto j : centergroup ) // loop over main group
                    if ( i != j )
                    {
        ...
        template<class Tspace>
            class ClusterMoveRef : public TranslateRotate<Tspace>
        {
            protected:
                typedef TranslateRotate<Tspace> base;
                typedef typename Tspace::ParticleVector Tpvec;
            ...
            public:
                using base::spc;
                ClusterMoveRef( Energy::Energybase<Tspace> &, Tspace &, Tmjson &j );
                virtual ~ClusterMoveRef();
                vector<double> threshold;        //!< Distance between particles to define a cluster
        };

        template<class Tspace>
            ClusterMoveRef<Tspace>::ClusterMoveRef(
                    Energy::Energybase<Tspace> &e, Tspace &s, Tmjson &j ) : base(e, s, j)
            {
                base::title = "ClusterRef " + base::title;
                base::cite = "doi:10/cj9gnn";
                gstatic.resize(spc->molecule.size());
                threshold.resize(spc->molecule.size());
                dp_rot.resize(spc->molecule.size());
                ...
                for ( auto &i : this->mollist )
                {
                    ...
                    dp_trans.at(int(i.first)) = m[centername].at("dp");
                    dp_rot.at(int(i.first)) = m[centername].at("dprot");
                    spread_cluster.at(int(i.first)) = false; //m[centername].value("spread",true); // always false
                    Point temp(0,0,0);
                    temp  << j.value("dir", string("1 1 1"));
                    dir.at(int(i.first)) = temp;  // magic!
                    temp  << j.value("dir2", string("1 1 1"));
        ...
        template<class Tspace>
            string ClusterMoveRef<Tspace>::_info()
            {
                using namespace textio;
                std::ostringstream o;
                o << base::_info() << endl;
        ...
        template<class Tspace>
            void ClusterMoveRef<Tspace>::getClusterAroundMolecule(Group *g)
            {
                int cntI = 0;
                for ( auto &i : spc->molList() ) { 	// For every type of molecule ...
                    bool is_static = false; 	// Start to assume the molecule is not prohibited from being in cluster around molecule 'g'
        ...
        template<class Tspace>
            void ClusterMoveRef<Tspace>::_trialMove()
            {
                // if `mollist` has data, favor this over `setGroup()`
                // Note that `currentMolId` is set by Movebase::move()
                if ( !this->mollist.empty())
                ...
                if ( dp_trans.at(int(this->currentMolId)) > 1e-6 ) {
                    for ( auto i : cindex ) {
			Point n = spc->geo.vdist(cm, i->massCenter(*spc));
                        i->translate(*spc,2.0*n);
			break;
		    }
                }
            }
        ...
        template<class Tspace>
            void ClusterMoveRef<Tspace>::_acceptMove()
            {
                base::_acceptMove();
                for ( auto k : cindex ) {
                    k->accept(*spc);
        ...
        template<class Tspace>
            void ClusterMoveRef<Tspace>::_rejectMove()
            {
                base::_rejectMove();
                for ( auto k : cindex ) {
                    k->undo(*spc);
        ...
        template<class Tspace>
            double ClusterMoveRef<Tspace>::_energyChange()
            {
                double bias = 1;             // cluster bias -- see Frenkel 2nd ed, p.405
...
#pragma omp parallel for reduction (+:du)
         ...
         */
        template<class Tspace>
            double ClusterMoveRef<Tspace>::ClusterProbability(Group &centergroup, Tpvec &p, int i )
            {
                for ( auto j : centergroup ) // loop over main group
                    if ( i != j )
                    {
        ...
        template<class Tspace>
            class ClusterMoveLarge : public TranslateRotate<Tspace>
        {
            protected:
                typedef TranslateRotate<Tspace> base;
                typedef typename Tspace::ParticleVector Tpvec;
            ...
            public:
                using base::spc;
                ClusterMoveLarge( Energy::Energybase<Tspace> &, Tspace &, Tmjson &j );
                virtual ~ClusterMoveLarge();
                vector<double> threshold;        //!< Distance between particles to define a cluster
        };

        template<class Tspace>
            ClusterMoveLarge<Tspace>::ClusterMoveLarge(
                    Energy::Energybase<Tspace> &e, Tspace &s, Tmjson &j ) : base(e, s, j)
            {
                base::title = "ClusterLarge " + base::title;
                base::cite = "doi:10/cj9gnn";
                gstatic.resize(spc->molecule.size());
                threshold.resize(spc->molecule.size());
                dp_rot.resize(spc->molecule.size());
        ...
        template<class Tspace>
            string ClusterMoveLarge<Tspace>::_info()
            {
                using namespace textio;
                std::ostringstream o;
                o << base::_info() << endl;
        ...
        template<class Tspace>
            void ClusterMoveLarge<Tspace>::getClusterAroundMolecule(Group *g)
            {
                int cntI = 0;
                for ( auto &i : spc->molList() ) { 	// For every type of molecule ...
                    bool is_static = false; 	// Start to assume the molecule is not prohibited from being in cluster around molecule 'g'
        ...
        template<class Tspace>
            void ClusterMoveLarge<Tspace>::_trialMove()
            {
                // if `mollist` has data, favor this over `setGroup()`
                // Note that `currentMolId` is set by Movebase::move()
                if ( !this->mollist.empty())
        ...
        template<class Tspace>
            void ClusterMoveLarge<Tspace>::_acceptMove()
            {
                base::_acceptMove();
                for ( auto k : cindex ) {
                    k->accept(*spc);
        ...
        template<class Tspace>
            void ClusterMoveLarge<Tspace>::_rejectMove()
            {
                base::_rejectMove();
                for ( auto k : cindex ) {
                    k->undo(*spc);
        ...
        template<class Tspace>
            double ClusterMoveLarge<Tspace>::_energyChange()
            {
                double bias = 1;             // cluster bias -- see Frenkel 2nd ed, p.405
...
#pragma omp parallel for reduction (+:du)
         ...
         */
        template<class Tspace>
            double ClusterMoveLarge<Tspace>::ClusterProbability(Group &centergroup, Tpvec &p, int i )
            {
                for ( auto j : centergroup ) // loop over main group
                    if ( i != j )
                    {
...
#pragma omp parallel for reduction (+:du) schedule (dynamic)
         ...
         * - External potentials are ignored
         *
         * @note Modification of 'ClusterTranslateNR'
         * @todo Energy calc. before and after can be optimized by only looping over `moved` with
         * `remaining`
         */
        template<class Tspace>
            class ClusterTranslateNR_ref : public Movebase<Tspace>
        {
            private:
                typedef Movebase<Tspace> base;
                using base::w;
                ...
                vector<Group *>
                    g;         //!< Group of molecules to move. Currently this needs to be ALL groups in the system!!
            public:
                ClusterTranslateNR_ref( Energy::Energybase<Tspace> &, Tspace &, Tmjson & );
                bool skipEnergyUpdate;    //!< True if energy updates should be skipped (faster evaluation!)
        };

        /** @brief Constructor */
        template<class Tspace>
            ClusterTranslateNR_ref<Tspace>::ClusterTranslateNR_ref( Energy::Energybase<Tspace> &e, Tspace &s, Tmjson &j ) : base(e, s)
        {
            auto _j = j;
            base::title = "Rejection Free Cluster Translation Reflection";
            base::cite = "doi:10/fthw8k";
            base::useAlternativeReturnEnergy = true;
            base::runfraction = _j["prob"] | 1.0;
            skipEnergyUpdate = _j["skipenergy"] | false;
        ...
        template<class Tspace>
            string ClusterTranslateNR_ref<Tspace>::_info()
            {
                using namespace textio;
                std::ostringstream o;
                o << pad(SUB, w, "Displacement") << dp << _angstrom << endl
        ...
        template<class Tspace>
            void ClusterTranslateNR_ref<Tspace>::_trialMove()
            {
                double du = 0;
                g = spc->groupList();
                moved.clear();
...
#pragma omp parallel for reduction (+:du) schedule (dynamic)
                ...
                int f = slump() * remaining.size(); // pick random molecule
                moved.push_back(remaining[f]);
                remaining.erase(remaining.begin() + f);    // Pick first index in m to move
		
                Point ip(dp, dp, dp);
		ip = spc->geo.len*slump.half();
                //ip.x() *= slump.half();
                //ip.y() *= slump.half();
                //ip.z() *= slump.half();
		//ip += g[moved[0]]->massCenter(*spc);
		Point n(1.0,0.0,0.0);
                ...
                for ( size_t i = 0; i < moved.size(); i++ )
                {
		    n = spc->geo.vdist(ip, g[moved[i]]->massCenter(*spc));
                    g[moved[i]]->translate(*spc, 2.0*n);
		    n = n/n.norm();
		    g[moved[i]]->reflect(*spc, n);
                    for ( size_t j = 0; j < remaining.size(); j++ )
                    {
                        double uo = pot->g2g(spc->p, *g[moved[i]], *g[remaining[j]]);
                        double un = pot->g2g(spc->trial, *g[moved[i]], *g[remaining[j]]);
...
#endif
            {
                ...
                for ( auto i = m.begin(); i != m.end(); ++i )
                {
                    ...
                    try {
                        ...
                        if ( i.key() == "moltransrotLarge" )
                            mPtr.push_back(toPtr(TranslateRotateLarge<Tspace>(e, s, val)));
                        ...
                        if ( i.key() == "ClusterMove2" )
                            mPtr.push_back(toPtr(ClusterMove2<Tspace>(e, s, val)));
                        if ( i.key() == "ClusterMove3" )
                            mPtr.push_back(toPtr(ClusterMove3<Tspace>(e, s, val)));
                        if ( i.key() == "ClusterMoveLarge" )
                            mPtr.push_back(toPtr(ClusterMoveLarge<Tspace>(e, s, val)));
                        if ( i.key() == "ClusterMoveRef" )
                            mPtr.push_back(toPtr(ClusterMoveRef<Tspace>(e, s, val)));
                        if ( i.key() == "isobaric" )
                        ...
                        if ( i.key() == "ctransnr_ref" )
                            mPtr.push_back(toPtr(ClusterTranslateNR_ref<Tspace>(e, s, val)));
                        if ( i.key() == "xtcmove" )
