...
namespace Faunus {
	 ...
	 */
	class TubeparticlePOVRay {
		public:

			template<class Tspace>
				void saveTubeparticlePOVRay(const string &filename, const Tspace &spc) const {
					std::ofstream f;
					f.open(filename);
					f << "#include \"colors.inc\"\n\n";

					Point camera_at = spc.geo.len;
					f << "#declare locX=" << camera_at.x() << ";\n";
					f << "#declare locY=" << camera_at.y() << ";\n";
					f << "#declare locZ=" << camera_at.z() << ";\n\n";
					f << "#declare colorLock=color rgb<0.05, 0, 0.8>;\n";
					f << "#declare colorLockCavity=color rgb<0.8, 0.05, 0>;\n";
					f << "#declare colorKey=color rgb<0.75, 0.75, 0>;\n\n";
					f << "#declare ambientv=0.3;\n"; // Light on particles, 0.0 = little light, 1.0 = much light
					f << "#declare phongv=0.1;\n\n";

					f << "background { color rgb <0.0, 0.0, 0.0> }\n\n";
					f << "camera {\n";
					f << "location <locX,locY,locZ>\n";
					...
					for(unsigned int i = 0; i < spc.p.size(); i++) {
						...
						if(spc.p[i].is_sphere()) {
							...
							f << "<" << xyz.x() << "," << xyz.y() << "," << xyz.z() << ">\n";
							f << "," << spc.p[i].radius << "\n";
							f << "texture { pigment { colorKey } }\n";
							f << "finish {ambient ambientv\n phong phongv }\n";
							f << "}\n";
							continue;
						}

						Point dir = spc.p[i].direction()*spc.p[i].height_tube();
						f << "cylinder {\n";
						f << "<" << xyz.x()-dir.x() << "," << xyz.y()-dir.y() << "," << xyz.z()-dir.z() << ">,\n";
						f << "<" << xyz.x()+dir.x() << "," << xyz.y()+dir.y() << "," << xyz.z()+dir.z() << ">,\n";
						f << spc.p[i].radius_tube() << " \n";
						f << "open \n";
						f << "texture { pigment { colorLock } }\n";
						f << "finish {ambient ambientv\n phong phongv }\n";
						f << "}\n";
					}
					f.close();
			...
			std::ostringstream getSphere(const Point& xyz, double radius) const {
				std::ostringstream sphi;
				sphi << "sphere {\n";
				sphi << "<" << xyz.x() << "," << xyz.y() << "," << xyz.z() << ">\n";
				sphi << "," << radius << "\n";
				sphi << "}\n";
				return sphi;
			}

			std::ostringstream getTube(const Point& xyz, const Point& dir, double height, double radius) const {
				std::ostringstream tube;
				Point dirl = dir*height;
				tube << "cylinder {\n";
				tube << "<" << xyz.x()-dirl.x() << "," << xyz.y()-dirl.y() << "," << xyz.z()-dirl.z() << ">,\n";
				tube << "<" << xyz.x()+dirl.x() << "," << xyz.y()+dirl.y() << "," << xyz.z()+dirl.z() << ">,\n";
				tube << radius << " \n";
				tube << "open \n";
				tube << "texture { pigment { colorLock } }\n";
				tube << "finish {ambient ambientv\n phong phongv }\n";
				tube << "}\n";
				return tube;
			}

			template<class Tspace>
				void saveTubeparticleDifferencePOVRay(const string &filename, const Tspace &spc) const {
					std::ofstream f;
					f.open(filename);
					f << "#include \"colors.inc\"\n\n";
					...
					for(unsigned int i = 0; i < spc.p.size()-1; i++) {
						...
						if(spc.p[i].is_sphere()) {
							first = getSphere(spc.p[i],spc.p[i].radius);
						} else {
							first = getTube(spc.p[i],spc.p[i].direction(),spc.p[i].height_tube(),spc.p[i].radius_tube());
						}
						for(unsigned int j = i+1; j < spc.p.size(); j++) {
							std::ostringstream second;
							if(spc.p[j].is_sphere()) {
								second = getSphere(spc.p[j],spc.p[j].radius);
							} else {
								second = getTube(spc.p[j],spc.p[j].direction(),spc.p[j].height_tube(),spc.p[j].radius_tube());
							}
							f << "intersection {\n" << first.str() << "\n " << second.str() << "}\n";
						}
					}
	...
	class KcPOVRay {
		public:
			...
			template<class Tspace>
				void saveKcPOVRay(const string &filename, const Tspace &spc) const {
					std::ofstream f;
					f.open(filename);
					f << "#include \"colors.inc\"\n\n";

					Point camera_at = Point(0.0,2.0*spc.geo.getLength(),0.0);
					f << "#declare locX=" << camera_at.x() << ";\n";
					f << "#declare locY=" << camera_at.y() << ";\n";
					f << "#declare locZ=" << camera_at.z() << ";\n\n";
					f << "#declare colorSide=color rgb<1.0, 1.0, 0.0>;\n";
					f << "#declare colorTop=color rgb<1.0, 0.0, 0>;\n";
					f << "#declare colorCyl=color rgb<0.05, 0.05, 0.05>;\n\n";
					f << "#declare ambientv=0.3;\n"; // Light on particles, 0.0 = little light, 1.0 = much light
					f << "#declare phongv=0.1;\n\n";
					...
					f << "cylinder {\n";
					f << "<0,0," << -spc.geo.getLength()/2.0 << ">, <0,0," << spc.geo.getLength()/2.0 << "> ";
					f << "," << spc.geo.getRadius() << "\n";
					f << "texture { pigment { colorCyl filter 0.9 } }\n"; // 0.9
					f << "finish {ambient 0.1 \n phong 0.1 }\n";
					f << "}\n";
					
					for(unsigned int i = 0; i < spc.p.size(); i++) {
						Point xyz = spc.p[i];
						Point lv = spc.p[i].lv()*spc.p[i].length()/2.0;
						Point wv = spc.p[i].wv()*spc.p[i].width()/2.0;
						Point dv = spc.p[i].dv()*spc.p[i].depth()/2.0;
						
						f << "triangle {\n";
						f << "  <" << xyz.x()+lv.x()+wv.x()+dv.x() << "," << xyz.y()+lv.y()+wv.y()+dv.y() << "," << xyz.z()+lv.z()+wv.z()+dv.z() << ">\n";
						f << ", <" << xyz.x()+lv.x()+wv.x()-dv.x() << "," << xyz.y()+lv.y()+wv.y()-dv.y() << "," << xyz.z()+lv.z()+wv.z()-dv.z() << ">\n";
						f << ", <" << xyz.x()+lv.x()-wv.x()+dv.x() << "," << xyz.y()+lv.y()-wv.y()+dv.y() << "," << xyz.z()+lv.z()-wv.z()+dv.z() << ">\n";
						f << "texture { pigment { colorTop } }\n";
						f << "finish {ambient ambientv \n phong phongv }\n";
						f << "}\n";
	...
	class CapparticlePOVRay {
		public:
			...
			template<class Tspace>
				void saveCapparticlePOVRay2(const string &filename, const Tspace &spc) const {
					std::ofstream f;
					f.open(filename);
					f << "#include \"colors.inc\"\n\n";
					...
					for(unsigned int i = 0; i < spc.p.size(); i++) {
						...
						for(int k = 0; k < spc.p[i].nbrOfCav(); k++) {
							Point cap = spc.p[i].cap_center_point(k)*spc.p[i].cap_center(k);
							//cout << "C: " << cap << ", " << spc.p[i].cap_center_point(k) << ", " << spc.p[i].cap_center(k) << endl;
							f << "sphere {\n";
							f << "<" << (xyz.x() + cap.x()) << "," << (xyz.y() + cap.y()) << "," << (xyz.z() + cap.z()) << ">\n";
							f << "," << spc.p[i].cap_radius(k) << "\n";
							f << "texture { pigment { colorLockCavity } }\n";
							f << "finish {ambient ambientv\n phong phongv }\n }\n";
						}
			...
			template<class Tspace>
				void saveCapparticlePOVRay(const string &filename, const Tspace &spc) const {
					std::ofstream f;
					f.open(filename);
					f << "#include \"colors.inc\"\n\n";
			...
			template<class Tspace>
				void saveDipCapParticlePOVRay(const string &filename, const Tspace &spc) const {
					std::ofstream f;
					f.open(filename);
					f << "#include \"colors.inc\"\n\n";
					...
					f << "#declare colorLockCavity=color rgb<0.8, 0.05, 0>;\n";
					f << "#declare colorKey=color rgb<0.75, 0.75, 0>;\n\n";
					f << "#declare colorDipA=Cyan;\n";
					f << "#declare colorDipB=Orange;\n\n";
					f << "#declare ambientv=0.3;\n"; // Light on particles, 0.0 = little light, 1.0 = much light
					f << "#declare phongv=0.1;\n\n";
					...
					for(unsigned int i = 0; i < spc.p.size(); i++) {
						Point xyz = spc.p[i];
						Point ocd = spc.p[i].off_center_dipole();
						Point mu = spc.p[i].mu();
						double radius = spc.p[i].radius;

						f << "sphere {\n";
						f << "<" << xyz.x()+ocd.x()+mu.x()*radius*scale << "," << xyz.y()+ocd.y()+mu.y()*radius*scale << "," << xyz.z()+ocd.z()+mu.z()*radius*scale << ">\n";
						f << "," << radius*scale << "\n";
						f << "texture { pigment { colorDipA transmit 0.1 } }\n";
						f << "finish {ambient ambientv \n phong phongv }\n";
						f << "}\n";
						f << "sphere {\n";
						f << "<" << xyz.x()+ocd.x()-mu.x()*radius*scale << "," << xyz.y()+ocd.y()-mu.y()*radius*scale << "," << xyz.z()+ocd.z()-mu.z()*radius*scale << ">\n";
						f << "," << radius*scale << "\n";
						f << "texture { pigment { colorDipB transmit 0.1 } }\n";
						f << "finish {ambient ambientv \n phong phongv }\n";
						f << "}\n";
						...
						if(spc.p[i].is_sphere()) {
							...
							f << "<" << xyz.x() << "," << xyz.y() << "," << xyz.z() << ">\n";
							f << "," << radius << "\n";
							f << "texture { pigment { colorKey transmit 0.3 } }\n";
							f << "finish {ambient ambientv \n phong phongv }\n";
							f << "}\n";
							continue;
						}
						...
						f << "<" << xyz.x() << "," << xyz.y() << "," << xyz.z() << ">\n";
						f << "," << radius << "\n";
						f << "texture { pigment { colorLock transmit 0.3 } }\n";
						f << "finish {ambient ambientv \n phong phongv }\n";
						f << "}\n";
						f << "sphere {\n";
						f << "<" << (xyz.x() + cap.x()) << "," << (xyz.y() + cap.y()) << "," << (xyz.z() + cap.z()) << ">\n";
						f << "," << spc.p[i].cap_radius() << "\n";
						f << "texture { pigment { colorLockCavity transmit 0.3 } }\n";
						f << "finish {ambient ambientv \n phong phongv }\n";
						f << "}\n}\n\n";
					}
					f.close();
			...
			template<class Tspace>
				void saveCapparticlePOVRayDifference(const string &filename, const Tspace &spc) const {
					std::ofstream f;
					f.open(filename);
					f << "#include \"colors.inc\"\n\n";
					...
					for(unsigned int i = 0; i < spc.p.size()-1; i++) {
						...
						std::ostringstream sphi;
						sphi << "sphere {\n";
						sphi << "<" << xyz_i.x() << "," << xyz_i.y() << "," << xyz_i.z() << ">\n";
						sphi << "," << spc.p[i].radius << "\n";
						sphi << "}\n";

						std::ostringstream capi;
						capi << "union {";
						Point cap_i = spc.p[i].cap_center_point()*spc.p[i].cap_center();
						capi << "sphere {\n";
						capi << "<" << (xyz_i.x() + cap_i.x()) << "," << (xyz_i.y() + cap_i.y()) << "," << (xyz_i.z() + cap_i.z()) << ">\n";
						capi << "," << spc.p[i].cap_radius() << "\n";
						capi << "}\n";
						capi << "}\n";
						...
						for(unsigned int j = i+1; j < spc.p.size(); j++) {
							Point xyz_j = spc.p[j];
							std::ostringstream sphj;
							sphj << "sphere {\n";
							sphj << "<" << xyz_j.x() << "," << xyz_j.y() << "," << xyz_j.z() << ">\n";
							sphj << "," << spc.p[j].radius << "\n";
							sphj << "}\n";

							std::ostringstream capj;
							capj <<"union {";
							Point cap_j = spc.p[j].cap_center_point()*spc.p[j].cap_center();
							capj << "sphere {\n";
							capj << "<" << (xyz_j.x() + cap_j.x()) << "," << (xyz_j.y() + cap_j.y()) << "," << (xyz_j.z() + cap_j.z()) << ">\n";
							capj << "," << spc.p[i].cap_radius() << "\n";
							capj << "}\n";
							capj << "}\n";

							std::ostringstream intj;
							intj << "difference {\n" << sphj.str() << "\n " << capj.str() << "}\n";
							f << "intersection {\n" << inti.str() << "\n " << intj.str() << "}\n";
						}
					}
			...
			template<class Tspace>
				void saveCapparticlePOVRayDifference2(const string &filename, const Tspace &spc) const {
					std::ofstream f;
					f.open(filename);
					f << "#include \"colors.inc\"\n\n";
					...
					for(unsigned int i = 0; i < spc.p.size()-1; i++) {
						...
						std::ostringstream capi;
						capi << "union {";
						for(int ci = 0; ci < spc.p[i].nbrOfCav(); ci++) {
							Point cap_i = spc.p[i].cap_center_point(ci)*spc.p[i].cap_center(ci);
							capi << "sphere {\n";
							capi << "<" << (xyz_i.x() + cap_i.x()) << "," << (xyz_i.y() + cap_i.y()) << "," << (xyz_i.z() + cap_i.z()) << ">\n";
							capi << "," << spc.p[i].cap_radius(ci) << "\n";
							capi << "}\n";
						}
						capi << "}\n";

						std::ostringstream inti;
						if(spc.p[i].nbrOfCav() > 0) {
							inti << "difference {\n" << sphi.str() << "\n " << capi.str() << "}\n";
						} else {
							inti << sphi.str();
						}
						...
						for(unsigned int j = i+1; j < spc.p.size(); j++) {
							...
							std::ostringstream capj;
							capj <<"union {";
							for(int cj = 0; cj < spc.p[j].nbrOfCav(); cj++) {
								Point cap_j = spc.p[j].cap_center_point(cj)*spc.p[j].cap_center(cj);
								capj << "sphere {\n";
								capj << "<" << (xyz_j.x() + cap_j.x()) << "," << (xyz_j.y() + cap_j.y()) << "," << (xyz_j.z() + cap_j.z()) << ">\n";
								capj << "," << spc.p[i].cap_radius(cj) << "\n";
								capj << "}\n";
							}
							capj << "}\n";

							std::ostringstream intj;
							if(spc.p[j].nbrOfCav() > 0) {
								intj << "difference {\n" << sphj.str() << "\n " << capj.str() << "}\n";
							} else {
								intj << sphj.str();
							}
