...
namespace Faunus
{
	...
	/** @brief Save system volume to disk. Keywords: `nstep`, `file` */
	class SystemVolume : public AnalysisBase {

	    std::ofstream f;
	    std::function<double(int)> volume;
	    int dim;
	    ...
	    public:
	    template<class Tspace>
		SystemVolume( Tmjson j, Tspace &spc ) : AnalysisBase(j)
	    {
		name = "System volume";
		string file = j.at("file");
		dim = j.at("dim");
		...
		volume = [&spc](int dim_in) { return spc.geo.getVolume(dim_in); };
	    }
	};
	...
	template<class Tspace>
	    class ChargeRDF : public PairFunctionBase {
		Tspace &spc;
		void update(data &d) override
		{
		    V += spc.geo.getVolume( d.dim );
			...
			for (int j=i+1; j<N; j++) {
			  double r = spc.geo.dist( spc.p[i], spc.p[j] );
			  d.hist(r) += (spc.p[i].charge + spc.p[i].charge);
			}
		    }
		}

		public:
		ChargeRDF( Tmjson j, Tspace &spc ) : PairFunctionBase(j,
			"Charge Distribution Function"), spc(spc) {}
	...
	template<class Tspace>
		...
		Table2D<double, double> table_rv1_a, table_rv2_a, table_v1v2_a, filled, unfilled, faceface, average_bond_angle;
		...
		Table3D<int, int > hist_su_3d, hist_mu_3d;
		double rcA_s, rcB_s;
		...
		void update( data &d ) override
		{
			...
			for ( int j = 0; j < N; j++ )
			{
			    ...
			    if(r1 > rcB)
				continue;
			    vectors.push_back(-r/r1);

			    (r1 < spc.p[i].radius + spc.p[j].radius) ? boundMatch++ : boundUnmatch++;
			    rv2 = acos(rv2); // convert 'cos(angle)' -> 'angle'
			    if( spc.p[i].is_sphere() ) {
				if( !spc.p[j].is_sphere() )
				    (pc::pi - spc.p[j].angle_p() > rv2) ? boundSpec++ : boundUnspec++; // i = sphere, j = cap
			    } else {
				rv1 = acos(rv1); // convert 'cos(angle)' -> 'angle'
				if( spc.p[j].is_sphere() ) {
				    (pc::pi - spc.p[i].angle_p() > rv1) ? boundSpec++ : boundUnspec++; // i = cap, j = sphere
				} else {
				    if(( pc::pi - spc.p[j].angle_p() > rv2) && ( pc::pi - spc.p[i].angle_p() > rv1 ) ) // i = cap, j = cap
					boundFaceface++;
				    ( ( pc::pi - spc.p[j].angle_p() > rv2) || ( pc::pi - spc.p[i].angle_p() > rv1 ) ) ? boundSpec++ : boundUnspec++;
				}
			    }
			}
			hist_mu_3d(boundMatch,boundUnmatch) += 1.0;
			hist_su_3d(boundSpec,boundUnspec) += 1.0;
			faceface(double(boundFaceface)) += 1.0;
			
			int vs = vectors.size();
			if(vs > 1) {
			  for(int j = 0; j < vs-1; j++) {
			    Point tmpA = vectors.at(j);
			    for(int k = j+1; k < vs; k++)
			      average_bond_angle(tmpA.dot(vectors.at(k))) += 1.;
			  }
			}
		    }
		}
		...
		Capanalysis( Tmjson j, Tspace &spc ) : PairFunctionBase(j,"Capanalysis"), spc(spc) {
		    ...
		    faceface.setResolution(1.0);
		    hist_mu_3d.setResolution(1,1);
		    hist_su_3d.setResolution(1,1);
		    ...
		    average_bond_angle.setResolution(datavec.back().dr);
		}
		...
		~Capanalysis()
		{
			...
			average_bond_angle.save( d.file+".aba" );

			faceface.normSave2(d.file+".faceface");
			hist_su_3d.save( d.file+".su.3d" );
			hist_mu_3d.save( d.file+".mu.3d" );
	...
	template<class Tspace>
	    class CapanalysisLK : public PairFunctionBase {
		Tspace &spc;
		Table2D<int, int> hist2_keys_in_lock, hist2_locks_on_key;
		Table3D<int, int > hist_keys_in_lock, hist_locks_on_key;

		void update( data &d ) override
		{
		    V += spc.geo.getVolume( d.dim );
		    int N = spc.p.size();
		    int id1 = atom[ d.name1 ].id; // Locks
		    int id2 = atom[ d.name2 ].id; // Keys
		    double sigma = atom[ d.name1 ].radius + atom[ d.name2 ].radius;
		    std::vector<int> keys_in_lock(N, 0);
		    std::vector<int> locks_on_keys(N, 0);
		    ...
		    nbrOfKeys = int(double(nbrOfKeys)/double(nbrOfLocks));
		    
		    
		    int max_nbr_of_keys_in_lock = *std::max_element(keys_in_lock.begin(), keys_in_lock.end());
		    std::vector<int> nbrs_keys_in_lock(max_nbr_of_keys_in_lock+1,0);
		    for(int i = 0; i <= max_nbr_of_keys_in_lock; i++ )
		      nbrs_keys_in_lock.at(i) = std::count(keys_in_lock.begin(), keys_in_lock.end(), i);
		    nbrs_keys_in_lock.at(0) -= nbrOfKeys;
		    
		    int max_nbr_of_locks_on_key = *std::max_element(locks_on_keys.begin(), locks_on_keys.end());
		    std::vector<int> nbrs_lock_on_key(max_nbr_of_locks_on_key+1,0);
		    for(int i = 0; i <= max_nbr_of_locks_on_key; i++ )
		      nbrs_lock_on_key.at(i) = std::count(locks_on_keys.begin(), locks_on_keys.end(), i);
		    nbrs_lock_on_key.at(0) -= nbrOfLocks;
		    
		    for(int i = 0; i < nbrs_keys_in_lock.size(); i++) {
		      hist2_keys_in_lock(i) += nbrs_keys_in_lock.at(i);
		      hist_keys_in_lock(i,nbrs_keys_in_lock.at(i)) += 1.0;
		    }
		    
		    for(int i = 0; i < nbrs_lock_on_key.size(); i++) {
		      hist2_locks_on_key(i) += nbrs_lock_on_key.at(i);
		      hist_locks_on_key(i,nbrs_lock_on_key.at(i)) += 1.0;
		    }
		    
		}

		public:
		CapanalysisLK( Tmjson j, Tspace &spc ) : PairFunctionBase(j,"CapanalysisLK"), spc(spc) {
		    hist2_keys_in_lock.setResolution(1);
		    hist2_locks_on_key.setResolution(1);
		    hist_keys_in_lock.setResolution(1,1);
		    hist_locks_on_key.setResolution(1,1);
		}

		~CapanalysisLK()
		{
		    for (auto &d : datavec) {
			hist2_keys_in_lock.save( d.file+".keys_in_lock.2d" );
			hist2_locks_on_key.save( d.file+".locks_on_key.2d" );
			hist_keys_in_lock.save( d.file+".keys_in_lock.3d" );
			hist_locks_on_key.save( d.file+".locks_on_key.3d" );
		    }
		}
	    };
	...
	template<class Tspace>
	    class AngleAnalyzis : public PairFunctionBase {
		Tspace &spc;
		double threshold;
		Table2D<double, double > table_angles;
		...
		void update( data &d ) override
		{
			...
			for ( int j = 0; j < N; j++ )
			{
			    ...
			    Point r_ji = spc.geo.vdist(spc.p[j], spc.p[i]);
			    double r_ji_1 = r_ji.norm();
			    if(r_ji_1 > 2.0*atom[ d.name1 ].radius + threshold )
			      continue;
				...
				Point r_ki = spc.geo.vdist(spc.p[k], spc.p[i]);
				double r_ki_1 = r_ki.norm();
				
				if(r_ki_1 > 2.0*atom[ d.name1 ].radius + threshold )
				  continue;
				
				// particles are bound
				
				double cosTheta = r_ji.dot(r_ki)/r_ji_1/r_ki_1;
				table_angles(cosTheta) += 1.0;
			    }
			}
		    }
		}
		...
		public:
		AngleAnalyzis( Tmjson j, Tspace &spc ) : PairFunctionBase(j,"AngleAnalyzis"), spc(spc) {
		    threshold = ( j.at("threshold") );
		    table_angles.setResolution(datavec.back().dr);
		}

		~AngleAnalyzis()
		{
		    for (auto &d : datavec) {
			table_angles.save( d.file+".angle" );
		    }
		}
	    };
	...
	class CombinedAnalysis : public AnalysisBase
	{
		...
		template<class Tspace, class Tpotential>
			...
			for ( auto i = m.begin(); i != m.end(); ++i )
			{
				...
				if ( i.key() == "volumefile" )
				    v.push_back(Tptr(new SystemVolume(val, spc)));
				...
				if ( i.key() == "capanalysisLK" )
				    v.push_back(Tptr(new CapanalysisLK<Tspace>(val, spc)));
				
				if ( i.key() == "capanalysisSS" )
				    v.push_back(Tptr(new Capanalysis<Tspace>(val, spc)));
				
				if ( i.key() == "capanalysisCS" )
				    v.push_back(Tptr(new Capanalysis<Tspace>(val, spc)));
				
				if ( i.key() == "capanalysisSC" )
				    v.push_back(Tptr(new Capanalysis<Tspace>(val, spc)));
				
				if ( i.key() == "capanalysisCC" )
				    v.push_back(Tptr(new Capanalysis<Tspace>(val, spc)));
				...
				if ( i.key() == "chargerdf" )
				    v.push_back(Tptr(new ChargeRDF<Tspace>(val, spc)));
				...
				if ( i.key() == "angleanalyzis" )
				    v.push_back(Tptr(new AngleAnalyzis<Tspace>(val, spc)));
