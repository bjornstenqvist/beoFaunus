        ...
        CoulombUniformCap::CoulombUniformCap( Tmjson &j ) : Coulomb(j)
        {
            ...
            for (auto &i : atom) {
                ...
                for (auto &j : atom) {
                    ...
                    double Rc = i.cap_radius;
                    double Rl = i.radius;
                    double c = i.cap_center;

		    // Get entities that are inherent in every particle
                    double angle_p = std::acos(-1.) - std::acos((Rl*Rl + c*c - Rc*Rc)/(2.0*Rl*c)); // Needs to be updated if 'capparticles' in file point.h is updated 
                    double angle_c = std::acos((c*c + Rc*Rc - Rl*Rl)/(2.0*c*Rc)); // Needs to be updated if 'capparticles' in file point.h is updated 
                    double area = std::acos(-1.)/c*(Rl*Rl*Rl + (Rc+2.0*c)*Rl*Rl - (Rc*Rc-c*c)*Rl-(Rc-c)*(Rc-c)*Rc); // Needs to be updated if 'capparticles' in file point.h is updated 

		    // Initiate temporary variables
                    double tmp_A = 0.0;
                    double tmp_B = 0.0;

		    // Calculate excate energy at the crossing between 'the line extrapolated between the center of the cap and the center of the particle' and 'the region II-III intersection-area' 
                    double z = c + Rc;
                    if(fabs(kappa) > 1e-9) { // kappa != 0
                        if(fabs(z) > 1e-9) {
                            tmp_A =  Rl/kappa/z*( exp( - kappa * ( Rl + z ) ) - exp( - kappa * sqrt( Rl*Rl + 2.0*Rl*z*cos(angle_p ) + z*z ))); // note that -cos(a.angle_p()) in faunus = cos(\alpha_p) from article
                        } else {
                            tmp_A = -Rl*(-cos(angle_p ) + 1.0)*exp(-kappa*Rl); // note that -cos(a.angle_p()) in faunus = cos(\alpha_p) from article
                        }

                        if(fabs(z-c) > 1e-9) {
                            tmp_B = -Rc/kappa/(c-z)*( exp( - kappa * ( Rc - (c - z) ) ) - exp( - kappa * sqrt( Rc*Rc - 2.0*Rc*(c-z)*cos(angle_c ) + (c-z)*(c-z) ) ) );
                        } else {
                            tmp_B = Rc*(cos(angle_c ) - 1.0)*exp(-kappa*Rc);
                        }
                    } else { // kappa == 0
                        if(fabs(z) > 1e-9) {
                            tmp_A = Rl/z*( - ( Rl + z ) + sqrt( Rl*Rl + 2.0*Rl*z*cos(angle_p ) + z*z ) ); // note that -cos(a.angle_p()) in faunus = cos(\alpha_p) from article
                        } else {
                            tmp_A = -Rl*(-cos(angle_p ) + 1.0); // note that -cos(a.angle_p()) in faunus = cos(\alpha_p) from article
                        }

                        if(fabs(z-c) > 1e-9) {
                            tmp_B = -Rc/(c-z)*( - ( Rc - (c - z) ) + sqrt( Rc*Rc - 2.0*Rc*(c-z)*cos(angle_c ) + (c-z)*(c-z) ) );
                        } else {
                            tmp_B = Rc*(cos(angle_c ) - 1.0);
                        }
                    }
                    double y2 = -(tmp_A+tmp_B)*lBxQQe(i.id, j.id)*2.0*pc::pi/area;

		    // Calculate excate energy at the crossing between 'the line extrapolated between the center of the cap and the center of the particle' and 'the region I-particle border-area'
                    z = c - Rc;
                    if(fabs(kappa) > 1e-9) { // kappa != 0
                        if(fabs(z) > 1e-9) {
                            tmp_A =  Rl/kappa/z*( exp( - kappa * ( Rl + z ) ) - exp( - kappa * sqrt( Rl*Rl + 2.0*Rl*z*cos(angle_p ) + z*z ))); // note that -cos(a.angle_p()) in faunus = cos(\alpha_p) from article
                    ...
                    double y1 = -(tmp_A+tmp_B)*lBxQQe(i.id, j.id)*2.0*pc::pi/area;

                    double x1 = exp(-kappa*Rl)/Rl;
                    double x2 = exp(-kappa*(c+Rc+Rl))/(c+Rc+Rl);
		    // Calculate 'slope' and y-intersect
                    double k = (y1 - y2)/(x1 - x2);
                    double m = y1 - k*x1;

                    regionII_k.seta(i.id, j.id, k );
                    regionII_m.seta(i.id, j.id, m );
                }
            }
        }
